...
append_block_averages <- function(df,
  ...
  req <- c("sub_id","balloon_color","trial_number","inflations","popped")
  ...
  membership <- df_blk %>%
    ...
    select(sub_id, balloon_color, block, block_label, n_all, n_adj, trials_all_str, trials_adj_str)
  
  # 4) Console sanity checks
  if (verbose) {
    cat("\n--- Block sanity checks (per participant / color / block) ---\n")
    membership %>%
      arrange(sub_id, balloon_color, block) %>%
      group_split(sub_id) %>%
      walk(function(tt) {
        pid <- tt$sub_id[1]
        cat(sprintf("\n[participant %s]\n", pid))
