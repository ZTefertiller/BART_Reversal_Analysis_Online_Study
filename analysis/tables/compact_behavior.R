...
#   - Writes:
#       "<out_prefix>_bysex_ttests.xlsx"
#       "<out_prefix>_collapsed.xlsx"
#   - Returns: list(by_sex = <df>, collapsed = <df>)
# Requirements in df:
#   participant_id, sex (0=male,1=female), session (1/2),
#   balloon_color (b/o/y/p), trial_number, inflations, popped (1/0)

build_compact_tables <- function(df, out_prefix = "bart_summary_compact") {
  # Guardrails
  need <- c("participant_id","sex","session","balloon_color",
  ...
  summarise_section_with_t <- function(d) {
    ...
    bind_rows(
      grp %>% filter(sex == "Men"),
      grp %>% filter(sex == "Women"),
      tt_rows
    ) %>%
      arrange(factor(color, levels = c("Blue","Orange","Yellow","Pink")),
              match(sex, c("Men","Women","t-test (M vs W)"))) %>%
      mutate(`Dependent measure` = paste(color, sex)) %>%
  ...
  final_bysex <- bind_rows(tab_pre, tab_post, tab_s2) %>%
  ...
  # ── Export ─────────────────────────────────────────────────────────────────
  # By-sex file
  f1 <- paste0(out_prefix, "_bysex_ttests.xlsx")
  wb1 <- createWorkbook()
  addWorksheet(wb1, "By sex + t-tests")
  writeData(wb1, "By sex + t-tests", final_bysex)
  hdr <- createStyle(textDecoration = "bold", halign = "center", border = "Bottom")
  addStyle(wb1, "By sex + t-tests", hdr, rows = 1, cols = 1:ncol(final_bysex), gridExpand = TRUE)
  left   <- createStyle(halign = "left")
  center <- createStyle(halign = "center")
  addStyle(wb1, "By sex + t-tests", left,   rows = 2:(nrow(final_bysex)+1), cols = 1:2, gridExpand = TRUE)
  addStyle(wb1, "By sex + t-tests", center, rows = 2:(nrow(final_bysex)+1), cols = 3:ncol(final_bysex), gridExpand = TRUE)
  setColWidths(wb1, "By sex + t-tests", cols = 1, widths = 28)
  setColWidths(wb1, "By sex + t-tests", cols = 2, widths = 24)
  setColWidths(wb1, "By sex + t-tests", cols = 3:ncol(final_bysex), widths = 18)
  saveWorkbook(wb1, f1, overwrite = TRUE)
  
  # Collapsed file
  f2 <- paste0(out_prefix, "_collapsed.xlsx")
  wb2 <- createWorkbook()
  addWorksheet(wb2, "Collapsed (no sex)")
  writeData(wb2, "Collapsed (no sex)", final_collapsed)
  addStyle(wb2, "Collapsed (no sex)", hdr, rows = 1, cols = 1:ncol(final_collapsed), gridExpand = TRUE)
  addStyle(wb2, "Collapsed (no sex)", left,   rows = 2:(nrow(final_collapsed)+1), cols = 1:2, gridExpand = TRUE)
  addStyle(wb2, "Collapsed (no sex)", center, rows = 2:(nrow(final_collapsed)+1), cols = 3:ncol(final_collapsed), gridExpand = TRUE)
  setColWidths(wb2, "Collapsed (no sex)", cols = 1, widths = 28)
  setColWidths(wb2, "Collapsed (no sex)", cols = 2, widths = 22)
  setColWidths(wb2, "Collapsed (no sex)", cols = 3:ncol(final_collapsed), widths = 18)
  saveWorkbook(wb2, f2, overwrite = TRUE)
  
  message("Wrote: ", f1)
  message("Wrote: ", f2)
  
  invisible(list(by_sex = final_bysex, collapsed = final_collapsed))
}
